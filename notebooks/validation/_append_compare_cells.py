"""Append 2D vs 3D comparison cells to 01 notebook. Run: python notebooks/validation/_append_compare_cells.py"""
import json
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
NB = ROOT / "notebooks" / "validation" / "01_embedding_vs_human_essentiality.ipynb"

MD = r"""## Native **D=2 vs D=3** on matched vertices
...
CODE = r"""# Matched D=2 vs D=3 comparison (run after setup cell with `ess`)
...
if any(
    c.get("cell_type") == "markdown" and any("D=2 vs D=3" in x for x in c.get("source", []))
    for c in nb["cells"]
):
    print("comparison section already present; skip")
    raise SystemExit(0)
...
nb["cells"].append({"cell_type": "markdown", "metadata": {}, "source": lines(MD)})
nb["cells"].append({"cell_type": "code", "metadata": {}, "outputs": [], "source": lines(CODE)})
NB.write_text(json.dumps(nb, indent=1, ensure_ascii=False), encoding="utf-8")
print("appended 2 cells ->", NB, "total", len(nb["cells"]))
