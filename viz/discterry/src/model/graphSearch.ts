...
/** Hyperbolic distance in the Poincaré ball between two W points (after focus map). */
export function poincareBallDistance(
  wx0: number,
  wy0: number,
  wz0: number,
  wx1: number,
  ...
  const r0 = wx0 * wx0 + wy0 * wy0 + wz0 * wz0;
  const r1 = wx1 * wx1 + wy1 * wy1 + wz1 * wz1;
  const dx = wx1 - wx0;
  const dy = wy1 - wy0;
  const dz = wz1 - wz0;
...
/** Prim MST on seed indices using pairwise hyperbolic distance in 𝔹³ at current W (after ball focus map). */
export function primHyperbolicSeedEdgesBall(
  wx: Float32Array,
  wy: Float32Array,
  wz: Float32Array,
  seedIndices: number[],
